...
void PlayScene::update()
{
	updateDisplayList();
	// close combat enemy
	//m_pSpaceShip->getTree()->getPlayerDetectedNode()->setDetected(false); // this would be different or A4
	...
	// Close Combat Enemy - Radius
	//if(m_pSpaceShip->getTree()->getPlayerDetectedNode()->getDetected() == false)
	//{
	//	bool isDetected = distance < 450; // Radius detection...just outside of LOS Range
	//	m_pSpaceShip->getTree()->getPlayerDetectedNode()->setDetected(isDetected);
	//}
...
void PlayScene::SpawnEnemyTorpedo()
{
	...
	// Set Direction of the torpedo (normalized - unit vector - value)
	...
	m_pTorpedoesK.back()->getTransform()->position = spawn_point; // place torpedo start at spawn_point
	SoundManager::Instance().playSound("torpedo_k"); // play the torpedo sound
	addChild(m_pTorpedoesK.back(), 2); // add torpedo to the scene
}
