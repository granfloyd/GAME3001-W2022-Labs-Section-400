...
void RangedCombatEnemy::LookWhereYoureGoing(const glm::vec2 target_direction, const float offset)
{
	float target_rotation = Util::signedAngle(getCurrentDirection(), target_direction) - offset;
...
void RangedCombatEnemy::MoveToLOS()
{
	...
	// Move to the closest node with LOS to the Target (without moving through an obstacle)
	// -- pathfinding
	// -- steering behaviours
}
...
void RangedCombatEnemy::Attack()
{
	...
	auto target_direction = Util::normalize(scene->getTarget()->getTransform()->position - getTransform()->position);
	LookWhereYoureGoing(target_direction, 0.0f);
